...
---Exec #Lntsp_Invoice_PO_WO_TAT @fromdate = '01-Jan-2017',@Todate = '30-Apr-2018'
...
select distinct b.DLRV_LR_Number,b.DLRV_MRN_Number,b.DLRV_MRN_Date, 
		C.MMAT_MG_Code, DMRN_Material_Code,MMGRP_Description, 
...
select  identity(int) slno,* into #MRNdetails1    from #MRNdetails

select * from #MRNdetails1 where slno between 0 and 900000

select * from #MRNdetails1 where slno > 900000
...
alter table #invoices add sbgdesc varchar (200)

alter table #invoices add Locdesc varchar(100)
alter table #invoices add city varchar (200)
alter table #invoices add state1 varchar (200)
alter table #invoices add jobnature varchar (100)




UPDATE a SET sbgdesc = b.SBG_Description
from #invoices a,lnt.dbo.job_master c, lnt.dbo.sbg_master b

WHERE a.JObcode=c.job_code and  c.sbg_code = b.sbg_code AND C.company_code ='LE'




UPDATE a SET Locdesc  = b.region_description
from #invoices a, lnt.dbo.region_master b,lnt.dbo.job_master c
WHERE a.JObcode=c.job_code  and c.location = b.region_code
and b. Company_Code= C.company_code

update a set city =UCITY_Name, state1=USTAT_Name
 from #invoices a, eip.SQLMAS.GEN_M_Address_Book, Eip.Sqlmas.Gen_M_Jobs,eip.sqlmas.GEN_U_States, Eip.Sqlmas.GEN_U_Cities
 where Mjob_AB_Code=MAB_AB_Code 
 and MAB_City_Code=UCITY_City_Code
 and UCITY_State_Code =USTAT_State_Code
 and A.JObcode=mjob_job_code

 update a set jobnature =b.type

 from #invoices a, lnt.dbo.job_nature_master b,lnt.dbo.job_master c
WHERE a.JObcode=c.job_code  and  C.job_nature = b.code

select mrdate, MrAuthDate, POAuth,DCDate, GinDate,	mrndate,MRNAuthDate,Invregisterdate,MinScrutinyDate,	MaxScrutinyDate,	JVDate,	MinDisbdate,	Maxdisbdate,	HPO_PO_Number,HPO_Job_Code,	BUdesc,	ICdesc,sbgdesc,Locdesc,city,state1,jobnature,	TLREG_LR_Number, invvalue	
...
JVDate,MinDisbdate,Maxdisbdate,BUdesc,ICdesc,DLRV_LR_Number,DLRV_WO_Bill_Number,JObcode,BUdesc,	ICdesc,sbgdesc,Locdesc,city,state1,jobnature,	HWO_WOT_Code, invvalue
...
and b.DLRV_WO_Bill_Number= hbill_bill_number
